...
readFile(void)
{
    ...
    int scanf_ret;
    unsigned short ub1, ub2;
    while ((scanf_ret = scanf("%c%s%hu%s%hu", &bufRule.proto,
                                              bufStr1,
                                              &ub1,
                                              bufStr2,
                                              &ub2)) != EOF) {
        ...
        if (bufStr2[0] == '0') {
            ...
            if (sscanf(bufStr1, "%hhu.%hhu.%hhu.%hhu", &bufRule.dst_ip[B_FST], &bufRule.dst_ip[B_SND],
        ...
        while (bufRule) {
            ...
            if (chkRule_src_port(bufRule->rule)) {
                for (int i = 0; i < PORT_BYTES; ++i) {
                    if (buf[OFFSET_PORT_SRC + i] != bufRule->rule.dst_ip[i]) flag = 0;
                }
            }

            if (chkRule_dst_port(bufRule->rule)) {
                for (int i = 0; i < PORT_BYTES; ++i) {
                    if (buf[OFFSET_PORT_DST + i] != bufRule->rule.dst_ip[i]) flag = 0;
                }
            }
