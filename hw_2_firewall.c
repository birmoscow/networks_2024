...
readFile(void)
{
    ...
    while ((scanf_ret = fscanf(rules, "%c%s%u%s%u\n", &bufRule.proto,
        ...
        if (bufStr1[0] == '0') {
            ...
            printf("OTL READ\n%hhu . %hhu . %hhu . %hhu\n", bufRule.src_ip[B_FST], bufRule.src_ip[B_SND],
                     bufRule.src_ip[B_TRD], bufRule.src_ip[B_FTH]);
        }
...
chkRule_src_port(Rule rule) {
    printf("B_FST %x  B_SND  %x\n\n", rule.src_port[B_FST], rule.src_port[B_SND]);
...
filter(int in, int out, List *rules)
{
    ...
    while (TRUE) {
        ...
        int flag = 0;
        while (bufRule) {
            printf("CHECKING BUF RULE\n");
            fflush(stdout);
            flag = 1;
            if (bufRule->rule.proto == 'u' && buf[OFFSET_IPV4_PROTO] != UDP) { flag = 0; printf("222\n");}
            if (bufRule->rule.proto == 't' && buf[OFFSET_IPV4_PROTO] != TCP) { flag = 0;  printf("223\n");}
            if (chkRule_src_ip(bufRule->rule)) {
                printf("IP SRC FRAME: %x %x %x %x\n", buf[OFFSET_IPV4_SRC], buf[OFFSET_IPV4_SRC + 1], buf[OFFSET_IPV4_SRC + 2], buf[OFFSET_IPV4_SRC + 3]);
                printf("RULE: %x %x %x %x\n", bufRule->rule.src_ip[0], bufRule->rule.src_ip[1], bufRule->rule.src_ip[2], bufRule->rule.src_ip[3]);
                for (int i = 0; i < IP_BYTES; ++i) {
                    if (buf[OFFSET_IPV4_SRC + i] != bufRule->rule.src_ip[i]) { flag = 0;  printf("226\n");}
                }
            }

            if (chkRule_dst_ip(bufRule->rule)) {
                printf("IP DST FRAME: %x %x %x %x\n", buf[OFFSET_IPV4_DST], buf[OFFSET_IPV4_DST + 1], buf[OFFSET_IPV4_DST + 2], buf[OFFSET_IPV4_DST + 3]);
                printf("RULE: %x %x %x %x\n", bufRule->rule.dst_ip[0], bufRule->rule.dst_ip[1], bufRule->rule.dst_ip[2], bufRule->rule.dst_ip[3]);
                for (int i = 0; i < IP_BYTES; ++i) {
                    if (buf[OFFSET_IPV4_DST + i] != bufRule->rule.dst_ip[i])  {flag = 0;  printf("232\n");}
                }
            }

            if (chkRule_src_port(bufRule->rule)) {
                for (int i = 0; i < PORT_BYTES; ++i) {
                    printf("From packet: %x\n", buf[OFFSET_PORT_SRC + i]);
                    if (buf[OFFSET_PORT_SRC + i] != bufRule->rule.src_port[i]) { flag = 0;  printf("237\n");}
                }
            }

            if (chkRule_dst_port(bufRule->rule)) {
                for (int i = 0; i < PORT_BYTES; ++i) {
                    if (buf[OFFSET_PORT_DST + i] != bufRule->rule.dst_port[i]) { flag = 0;  printf("244\n");}
                }
            }
